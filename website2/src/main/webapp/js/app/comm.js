KALAHA.comm = function() {
	...
	var _loginCB = function loginCallback(status, playerId, name) {
	    ...
	    if (status === 'OK') {
	    	_message("Login OK!");
	        _playerId = playerId;
	    	_instance.join($.kalaha.getGameId());
	    } else {
	...
	this.connect = function() {
		...
		if(_haveWebSocket()) {
			...
			_connector.connect("FIREBASE.WebSocketAdapter", __firebaseHost, __firebasePort, "/socket");
		} else {
			console.log("Connecing with CometD... ");
			_connector.connect("FIREBASE.CometdAdapter", __firebaseHost, __firebasePort, "/cometd", false, function() { return $.cometd });
		}
	};
}